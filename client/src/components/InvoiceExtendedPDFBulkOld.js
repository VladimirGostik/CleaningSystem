// InvoiceExtendedPDFBulkOld.js
//
// STARÝ FORMÁT FAKTÚRY (stav kódu z 3.3.2025, commit 59a9721) - použitý na
// dotlač starých faktúr tak, aby vyzerali presne ako pôvodne vytlačené.
// Oproti dnešnému InvoiceExtendedPDFBulk.js zámerne NEOBSAHUJE:
//   - QR kód (Pay by Square) ani riadok "Variabilný symbol"
//   - rámček okolo platobného bloku a absolútne umiestnené podpisy
// a zachováva pôvodné pravidlo {mesiac/rok} (mesiac 12 -> rok mínus 1).
// NEUPRAVOVAŤ podľa nových požiadaviek - zmeny patria do InvoiceExtendedPDFBulk.js.
...
const InvoiceExtendedPDFBulkOld = ({ invoice }) => {
  const {
    invoice_number,
    issue_date,
    due_date,
  ...
  // Pri dotlači starých faktúr musí v pätičke byť rok vystavenia faktúry,
  // nie aktuálny rok - aby dotlač sedela s pôvodne vytlačenou faktúrou.
  const issueYear = issue_date ? new Date(issue_date).getFullYear() : NaN;
  const footerYear = Number.isNaN(issueYear) ? new Date().getFullYear() : issueYear;
    ...
    <View style={styles.container}>
      ...
      <View style={styles.signatureSectionFull}>
        ...
        <View style={styles.footer}>
          <Text>
            © {footerYear} {company_name || 'N/A'}. Všetky práva vyhradené.
          </Text>
        </View>
      </View>
    </View>
...
InvoiceExtendedPDFBulkOld.propTypes = {
...
export default InvoiceExtendedPDFBulkOld;